 ...
 */
public class RdmaActiveEndpointGroup<C extends RdmaActiveClientEndpoint> extends RdmaEndpointGroup<C> {
	...
	void close(RdmaClientEndpoint endpoint) throws IOException {
		IbvContext context = endpoint.getIdPriv().getVerbs();
		RdmaActiveCqProcessor<C> cqProcessor = cqMap.get(context.getCmd_fd());
