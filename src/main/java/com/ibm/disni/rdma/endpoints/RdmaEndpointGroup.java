...
package com.ibm.disni.rdma.endpoints;
...
public abstract class RdmaEndpointGroup <C extends RdmaClientEndpoint> {
	...
	public synchronized final C createClientEndpoint() throws IOException {
		RdmaCmId idPriv = cmProcessor.createId(RdmaCm.RDMA_PS_TCP);
		C ep = createClientEndpoint(idPriv);
		ep.setServerSide(false);
		return ep;
	}
	
	protected synchronized final C createClientEndpoint(RdmaCmId idPriv)	throws IOException {
		C ep = factory.createClientEndpoint(idPriv);
	...
	public synchronized void close() throws IOException, InterruptedException {
		...
		LinkedList<RdmaClientEndpoint> clientEps = new LinkedList<RdmaClientEndpoint>();
		for (RdmaClientEndpoint ep : clientEndpointMap.values()) {
			clientEps.add(ep);
		}
		for (RdmaClientEndpoint ep : clientEps){
			ep.close();
		}
	...
	synchronized IbvPd createProtectionDomainRaw(RdmaClientEndpoint endpoint) throws IOException{
	...
	synchronized RdmaCqProvider createCqProviderRaw(RdmaClientEndpoint endpoint) throws IOException {
		return createCqProvider(clientEndpointMap.get(endpoint.getIdPriv()));
	}
	
	synchronized IbvQP createQpProviderRaw(RdmaClientEndpoint endpoint) throws IOException{
		return createQpProvider(clientEndpointMap.get(endpoint.getIdPriv()));
	}
	
	synchronized void allocateResourcesRaw(RdmaClientEndpoint endpoint) throws Exception {
		allocateResources(clientEndpointMap.get(endpoint.getIdPriv()));
	}	

	synchronized void unregisterClientEp(RdmaClientEndpoint endpoint) throws IOException {
