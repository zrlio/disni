 ...
 */
public class RdmaClientEndpoint{
	...
	protected RdmaEndpointGroup<? extends RdmaClientEndpoint> group;
	...
	protected RdmaClientEndpoint(RdmaEndpointGroup<? extends RdmaClientEndpoint> group, RdmaCmId idPriv) throws IOException{
	...
	public synchronized void dispatchCmEvent(RdmaCmEvent cmEvent)
		...
		try {
			...
			if (eventType == RdmaCmEvent.EventType.RDMA_CM_EVENT_ADDR_RESOLVED.ordinal()) {
				connState = RdmaClientEndpoint.CONN_STATE_ADDR_RESOLVED;
				notifyAll();
			} else if (cmEvent.getEvent() == RdmaCmEvent.EventType.RDMA_CM_EVENT_ROUTE_RESOLVED.ordinal()) {
				connState = RdmaClientEndpoint.CONN_STATE_ROUTE_RESOLVED;
				notifyAll();
